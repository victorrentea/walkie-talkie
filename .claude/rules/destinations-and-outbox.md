...
- **The footer is a legend keyed by those tokens** (`AppDelegate.artifactsClause`): `[📁=<folder>]` once and only when something is in it — the line that *defines* `📁` carries it (2026-09-20) — then a row per artifact. **The plain frames share one `[📸n = 📁/screenshot-n-800px.jpg at 800px width, or -original.jpg at 3456x2234px]`** since 2026-09-20, measured at 36/36 on naming a frame no row mentions; the conditions and the two tests are in `screenshots-and-selection.md`, *The footer folds the plain frames into one row*. A row that carries a clock never folds.
...
- **The destination terminal's own picture, 0.55 s, since 2026-09-23** — *"să plece cu poza terminalului până la locația în care e terminalul"*. A small copy of the bound terminal is born under the panel (`spawnSeed`, the spawn's grammar) and grows onto the window it is a copy of, arriving pixel for pixel. The picture is grabbed **by window id** (`BindFlight.windowPicture`, `carrying:`), off the main thread next to `terminalWindowFrame`, because the bound terminal is usually behind the app he dictated from and a screen-rect grab would photograph whatever covers it. The 09-07 rule (*"doar un chenar"*) was about the **panel's** picture covering the destination; the destination's own picture does not. **The outline is the fallback** when no picture comes back.
