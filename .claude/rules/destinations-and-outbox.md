...
## The status line's badge says *these words land here* (2026-09-16)

- **`~/.walkie-talkie/bound-tty` holds two facts on one line** — `ttys006`, or `ttys006 listening`. The status line wears a **yellow** badge for the first and a **red** one for the second. Written only from `AppDelegate.publishBinding`, which `showBound` and `syncBorrowedGestures` both pass through, so it cannot drift from the chip.
- **`listening` in that file means *the words are coming to this tty*, not *a microphone is open somewhere*.** A spawn (🔼 ↑) and a caret dictation (🔼 click) both run with a terminal still bound, and both used to turn that terminal's badge red for their whole length. Victor: *"nu mai vrea să înroșești terminalul care are microfonul conectat … mă induce în eroare, am impresia că există un terminal, când de fapt va ateriza într-o sesiune nouă"*. The badge stays **yellow** for them: the binding is still real, it is just not this sentence's destination.
- **The predicate is `listening && !spawnPending && !pasteMode`** — the same one `latchedAtCaret` is latched from when the microphone closes, so the badge and the delivery can never disagree. It is read as properties at call time rather than passed in, because `showBound`'s two mid-sentence branches (`clearSpawn` for a spawn bound mid-sentence, `pasteMode = false` for a caret one redirected at a terminal) take them back *before* `publishBinding`, which is what makes the badge go red again on the same call he redirects the sentence with.
- **`relay-restart.sh` may not use that word as *a dictation is running* any more.** It did, and the yellow spawn/caret sentence would have been restarted out from under him — the one thing that script exists to prevent. `relay_is_dictating` keeps the file as the cheap *yes* and asks `GET /test/state` for everything it no longer covers; an unreachable relay is a **no** there, or restarting a stopped app would wait for ever.
