...
- **Gated on *is a microphone open*, never on where the words go.** `syncMusic` is `music.setActive(listening || wisprHearing)`. It rode `live` (a bound sentence) until 2026-09-03 and `listening` alone until 2026-09-18; both narrowings were the same mistake — the pause is not about the destination. → journal: *The music pauses for the length of a dictation*, *The music also pauses for a dictation the relay did not start*
- **`wisprHearing` is the half that covers a dictation the relay did not start**, i.e. Victor holding right ⌘⌥ with the Engine on the local model — the commonest dictation of the day, and one `DictationSource`'s five events never reach, because only the *wired* source has them. It comes from `WisprFlowSource.hearingChanged`, wired once at launch beside `wrapWispr`, **not** in `wireDictationSource`. → journal: *The music also pauses for a dictation the relay did not start*
- **Take it from `WisprState.listening`, never from a `WisprWatch` boolean.** In that configuration `wisprSource.prepare()` is never called, so its `watch` is unstarted and the 100 ms poll samples an empty list; the CoreAudio notification is 0–6 s late and gave no edge at all in five of five runs on 2026-09-13. What fires is Wispr's `History` row — 182 ms, measured 2026-09-18. → journal: *The music also pauses for a dictation the relay did not start*
- **The push-to-talk sentence ends when the keys go up, and only the keyboard can say so.** `HotkeyTap.onWisprPushToTalkReleased` → `WisprFlowSource.pushToTalkReleased` → the ordinary `closeListening`. Wispr's row leaves `listening` only on a **terminal** status, so without this the music stayed off from the release until after the formatting pass — or for ever, if the row stopped at `raw_transcript` (measured 09:00–09:04, 2026-09-18). → journal: *The release of ⌘⌥ is the end of the sentence*
- **Never let a pause outlive the sentence.** `wisprHearing` only falls when the machine leaves `listening`, which depends on rows Wispr writes — so a Wispr that dies mid-sentence would mute his tabs for ever. `wisprHearingCeiling` (10 min) puts the music back and logs it as an **error**: reaching it is a bug, not a timeout. → journal: *The music also pauses for a dictation the relay did not start*
