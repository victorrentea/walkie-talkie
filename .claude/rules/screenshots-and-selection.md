...
- **The frame flies into the chip's `📸`; there is no vignette** (2026-09-23). Victor: *"takes that photo … and zooms it … into the icon that shows the number of photos. That can avoid you having to draw a yellow border around the screen."* Once `screencapture` returns (~150 ms), `AppDelegate.flyShotIntoChip` decodes the 800 px copy (for an area, the cut-out) and `CaptureFlash.flyIntoChip` shrinks it — white 4 pt edge thinning to 1, 0.5 s, below the chip's level — from the display (or the dragged box) onto `RelayWindow.shotLanding`, the glyph of the `📸 ×N` row it has just incremented. Main-queue order puts it behind `publishShotCount`, so the row is laid out first. No chip on screen, no flight. **Not for picture zero** (2026-09-23, Victor: *"poza inițială din gesturile de prompting nu o mai anima … pune doar bulina galbenă"*): the context shot taken by starting to talk gets only the tap ripple — no flight, no border. The flight is for shots he takes on purpose (⌃⌥P, the dragged area). **The dragged area flies its outline only** (2026-09-24, Victor: *"it's a bit distracting to see the image zooming in and collapsing inside, so just the border, the rectangle"*): `flyShotIntoChip(…, outlineOnly: true)` hands `flyIntoChip` a nil picture, so the white edge shrinks from the box into the `📸` with nothing inside it; the ⌃⌥P full-screen shot still carries its pixels. Reviewable from a desk with `POST /test/area`.
- **Mark first.** `CaptureFlash.announce()` at the top of `captureContext` / `plusOneShot`, before the selection probe and before `screencapture`, with the slow work on a background queue. (It fired the vignette too, until 2026-09-23.) Fired from inside `ScreenCapture.grab` it landed after a clipboard probe that sleeps up to 400 ms and a subprocess — a receipt that far behind the gesture no longer says *now*. Its panel is `sharingType = .none`, so firing first cannot put it in the shot. → journal: *Capture order: flash first*
