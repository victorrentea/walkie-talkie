...
- **`bind(tty:)` fails when nothing hosts the tty** (2026-09-26, test plan §3.4): a Terminal.app tab must show it (`liveTitles()`) or a tmux client be attached to it, else nil → `POST /bind` 409, restore retries and flashes. It used to always return a `Target`: `ttys999` and an IDE's pty were bound and the first sentence into them was lost with a receipt (TD1, TD14). → journal: *Fixes to the test plan's findings, batch 1*
...
- **Carriers are refused like a shell (2026-09-26, Victor's Q5: *refuse*).** `TerminalBinding.shellCarriers` — `ssh`, `mosh`, `sudo`, `su`, `doas`, `script`, `docker`, `screen`, `tmux`, `less`, `more`, `man`… — hand the line to a shell on another pty or host, so a job-local shell test never saw one: TD15 ran `touch` behind `script`, TD16's `q` quit `less` and zsh ran the rest. `refusesDelivery` = shell or carrier; `foregroundCommand` names a carrier first when one is anywhere in the job (`git log` → `less`). `claude`, `node`, `codex`, `cat` still pass. → journal: *Fixes to the test plan's findings, batch 1*
...
- **`activeAgentSessions()` is *where an agent is waiting*, not *every tab*** (2026-09-23, the spawn menu's *Active Terminals*): `liveTitles()` + one `ps -ax -o pid=,tty=` + `publishedDirectory(ownedBy:)` per pid on those ttys — the chip's own two guards (the `.last-<pid>` file postdates the process, the path is a directory). Never an `osascript` per tab. → `.claude/rules/spawn.md`, *Active Terminals*
...
- **⏎ on a row whose window is closed reopens the session** (2026-09-12): `SpawnTerminal.resumeClaude` → `claude --resume <id>` in the session's own folder, then the ordinary spawn tiling, flight and bind. Offered only while that folder exists — a session id is scoped to it and is not found from anywhere else. The hint (`⏎ resume it`, accent, right-aligned) is drawn on the **selected row only**, and the footer says the same thing: *"cu un visual hint, să ofere și asta"*. → journal: *A closed window is not a dead end: ⏎ reopens the session*
- **Never match a live tab on the folder name.** It calls every session of a repo live while any tab is open in it — a bind pointed at a stranger's window, and a search hit silently swallowed as a duplicate of a tty already listed. Only `ttyOwner` decides; the title corroborates. → journal: *A closed window is not a dead end: ⏎ reopens the session*
...
- **`POST /test/rebind-panel` `{"query": …}` and `POST /test/resume-session` `{"session": …, "directory": …}` are the only ways to reach any of this at a desk** — it is a window that takes the keyboard, opened from a menu row. → journal: *The panel's own corrections, in the order they were found*
...
- **Read `~/.walkie-talkie/bound-tty` before standing the app down** (launch clears it): absent unbound, `ttysNNN` bound. Whether a sentence is in flight is asked of `GET /test/state`, and the wait is **six seconds** after it goes idle — the decode and the held panel come after the microphone closes. → journal: *Restarting keeps the binding, and never interrupts a sentence (2026-09-09)*
...
- **`done` is not a sentence in flight, and a predicate that wants `phase == "idle"` hangs for ever** (2026-09-17). `DictationPhase` sits at `done(<status>)` from the moment a sentence lands until the next chord, so `relay-restart.sh` blocked five minutes on a relay that had finished. Both of its readings name the phases explicitly now: `warming` / `listening` / `transcribing` are a sentence, `idle` and `done` are not.
...
- **A bind while recording redirects the sentence; only a deliberate bind may do this.** Since 2026-09-26 (Victor's Q2) the close latches **which terminal** (`AppDelegate.latch` → `Message.target`) and `commit` delivers there (`TerminalBinding.deliver(_:to:)`) whatever was bound, rebound or unbound during the settle or the panel — TD4, TD29, TR24, TG19. A latched target found gone unbinds only if it *is* the binding (`unbind(ifAddress:)`). `showBound` takes back `spawnPending` (since 2026-08-31) and `pasteMode` (since 2026-09-09) on a running recording — but `refreshBoundTitle` rides the 10s poll and calls `showBound` too, and the take-back once fired on the poll: *"după 2-3 sec de vorbit, fără să apăs niciun buton, tooltipul a arătat că s-a reconectat"* (fixed 2026-09-02). `showBound(_:deliberate:)` carries the difference; the poll is the one caller passing `false`. Only the sentence converts, never `replaceWispr`. The recipient is whoever the relay points at when the microphone closes; a bind during the hold changes nothing. → journal: *A bind mid-sentence changes the recipient*
