...
- **`Transcribing via <logo>...` names the recogniser** (2026-09-22 evening), and the logo is a
  picture in the word exactly as the microphone is one row up. The mark rides `engineMark`, which
  until that evening lived on the `Listening` row: the two facts are true at two different moments
