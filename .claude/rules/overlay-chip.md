...
- **A row whose string carries a picture is measured off the label, never off the font** (2026-09-19). `measure(_:font:)` knows one font and cannot answer for a run that is an image, so `rowWidth` is gone and both the engine row and the ⌘-pick row call `sizeToFit`. Measured on the real chip the day the microphone glyph shipped: the row asked for **158** and needed **161.5** drawn — and a label 3.5pt short does not lose 3.5 points off the end, it drops the whole tail, so `Listening(🎙️/E)...` reached Victor as `Listening(🎙️/`. → journal: *The chip says which microphone, and the menu picks it (2026-09-19)*
...
| dictating | `Listening(<glyph>/<letter>)...` — **which microphone feeds which recogniser** (2026-09-19; the letter alone since 2026-09-18). 🎙️ Elgato Wave XLR · 💻 the built-in · 🎤 the DJI receiver · 🎧 Bose, and `(L)` local · `(E)` ElevenLabs · `(S)` Speechmatics · `(G)` Gemini · `(W)` Wispr. A device that is none of the four leaves the glyph out and the mark is the bare `(E)`. **Pushed in as a string by `AppDelegate`** — the chip may not ask which engine or which device it means (`RelayWindow.setEngineMark`), and it is re-read at every `dictationBegan`, because cables move between sentences. **The glyph is drawn as a picture** (`Glyphs.emoji` → `inline`, cached in `wordGlyphs`), never as a character in the string, and it is **always lit** while the letters ramp: a picture has no unlit state that reads as *not yet*, and dimming it by alpha is the halo problem the opaque grey exists to avoid |
...
- **The tally is the icon column read downwards**, each count two characters wide:
  ...
  **The quoted row is the exception to `×N`** (2026-09-14): it used to collapse to `×2` four
