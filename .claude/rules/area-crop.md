...
- **⇧ mid-drag is *this, moved to there*** (2026-09-24). Victor: *"the old shape will remain there, locked. Let's say source, and then an arrow should lay from the center of it to wherever the new shape … is … If I release Shift then the whole thing falls back to the previous shape."* Pressing ⇧ locks the box as the **source** (dashed) and starts a **second box at the pointer**, drawn corner to corner like the first — ⌘ and ⌥ act on it as they did on the first — with an arrow from the source's centre to its centre. Letting ⇧ go drops the second box and hands the source back to the hand at exactly its shape. Releasing the wheel **with ⇧ held** ends it as a move: `Selection.movedTo` (the kit, opted into with `CropSelectionStyle.allowsMove`; Victor Addons does not, so ⇧ still means nothing there). A destination under 6 pt (⇧ pressed, nothing drawn) is a plain selection of the source. The readout gains `⇧ move to`, lit while the second box is up.
