...
- **⇧ locks the box; the wheel draws where it goes; ⇧ need not be held** (2026-09-24, evening — supersedes the same morning's *⇧ held for the whole second box*). Victor: *"currently while holding shift down i also have to keep holding wheel … while shift down, an arrow from center of first rectangle should follow my mouse, then stay on center of 2nd box when drawn by wheel drag"*; the lock is sticky, *"esc cancels all"*, it *"finishes when I release the second box"*, and not Caps Lock. The kit's `MovePhase`: **pointing** — ⇧ pressed with the wheel still down locks the box as the source (dashed, only if ≥ 6 pt), the arrow runs from its centre to the pointer; ⇧ let go here hands the box back to the hand. **parked** — the wheel released with the source locked: sticky, ⇧ may go, the arrow keeps following, the pill says `wheel-drag where it goes · Esc cancels`; `onAwaitingDestination(true)` tells the tap. **drawing** — the next wheel press is claimed by `HotkeyTap.areaDrag` **on sight** (`areaAwaitingDestination`: no 12 pt threshold, modifiers allowed, press and release both swallowed — the overlay covers the screen, there is no tab under it) and handed in through `destinationPressed(atCG:)`; the box is drawn corner to corner with ⌘/⌥ as usual and the arrow's tip sits on its centre. Its release ships `Selection.movedTo`; a destination under 6 pt (a click) parks again. Esc or the right button cancels from any phase; the sentence ending (`syncBorrowedGestures`, `!live`) takes a parked selection down (`cancelIfParked`). Victor Addons does not opt in (`allowsMove`), so ⇧ still means nothing there. **Known gap:** the desktop-effects hold is 20 s, so a selection parked longer than that lets the ☕ reactions back in.
- **What a move ships: one clean frame, both boxes in words.** `grabArea(…, movedTo:)` measures the second box into the same pixels and remembers it by path (`ScreenCapture.moveTarget(for:)`, in memory — the envelope is written by this process minutes later at most); the `-zoom` cut-out is the **source**, the thing being moved. The token reads `[📸3✂️x1,y1→x2,y2 moved to x1,y1→x2,y2]`, the footer row `user-drawn MOVE: what is in the box between corners (x,y) (…)→(…) should go to the box (…)→(…); the first box cut out at …`. **Nothing is drawn into the pixels** — the arrow is on screen only, like the cursor mark. The receipt outline flies from the destination. `POST /test/area {"to": {x,y,w,h}}` is the desk route; `evals/test_envelope.py::MovedArea` asserts it.
