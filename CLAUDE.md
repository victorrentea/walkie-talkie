...
| `POST /test/paste-hint` | one showing of the **`⌘⇧P` hint** under the pointer (2026-09-22; 3 s at 80% since 2026-09-23) — the mark itself, without having to deliver a sentence to see it. `GET /test/state.pasteHint` reads it back; `WT_SHOOT_HINT=<path>` draws it on both grounds with no app running |
...
- **Keys:** ⌘⌃B binds the terminal in front (again on the same target: unbinds), ⌘⌃D starts /
  ...
  **⌘⇧P is shown after every delivered sentence** (2026-09-23, superseding 2026-09-22's *caret
  ...
  plasez greșit, lasă-mă să-mi amintesc constant"*. `PasteHint`: **0.80** at once for
  **2.5 s** (`hold`), faded out over **0.5 s** (`fall`), `WT_PASTE_HINT_PEAK` for a run; placed
  once and never following. A sentence held for a bind gets it at its release, not at the hold.
