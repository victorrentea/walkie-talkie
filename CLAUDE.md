...
| `POST /test/paste-hint` | one pulse of the **`⌘⇧P` hint** under the pointer (2026-09-22) — the mark itself, without having to lose a sentence to see it. `GET /test/state.pasteHint` reads it back; `WT_SHOOT_HINT=<path>` draws it on both grounds with no app running |
