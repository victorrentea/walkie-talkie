...
# Vocea de referință, nu zgomotul generat — vezi `assets/halo-ref-voice.txt`.
WAV=${WAV:-$(pwd)/assets/halo-ref-voice.wav}
...
for S in $STYLES; do
  for R in $ROUTES; do case $R in
    web)     WT_HALO_ENGINE=web    WT_HALO_STYLE=$S WT_MD_SHOOT=$OUT/$S-web     WT_HALO_DEMO=10 WT_HALO_DEMO_AUDIO="clip:$WAV" WT_ALLOW_DIRECT=1 ./.build/debug/WalkieTalkie > $OUT/$S-web.log 2>&1 ;;
    native1) WT_HALO_ENGINE=native WT_HALO_STYLE=$S WT_PM_SHOOT=$OUT/$S-native1 WT_PM_SCALE=1 WT_HALO_DEMO=10 WT_HALO_DEMO_AUDIO="clip:$WAV" WT_ALLOW_DIRECT=1 ./.build/debug/WalkieTalkie > $OUT/$S-native1.log 2>&1 ;;
    native2) WT_HALO_ENGINE=native WT_HALO_STYLE=$S WT_PM_SHOOT=$OUT/$S-native2 WT_PM_SCALE=2 WT_HALO_DEMO=10 WT_HALO_DEMO_AUDIO="clip:$WAV" WT_ALLOW_DIRECT=1 ./.build/debug/WalkieTalkie > $OUT/$S-native2.log 2>&1 ;;
  esac; done
done
ls $OUT/*.png
